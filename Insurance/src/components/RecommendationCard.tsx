...
export const RecommendationCard: React.FC<RecommendationCardProps> = ({
  ...
  const isCurrentlySelected = selectedAmount === recommendedAmount;
  const explanation = getRecommendationExplanation(dailyEarnings, recommendedAmount);
  const monthlyAmount = calculateMonthlyContribution(recommendedAmount, activeWorkingDays);
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {dailyEarnings !== null && (
                <span className="text-xs text-stone-600 font-medium">
                  Based on verified ₹{dailyEarnings}/day wage
                </span>
              )}
            </div>
            <h3 className="text-lg sm:text-xl font-black text-stone-900 mt-1">
              Recommended: ₹{recommendedAmount} / day ({explanation.tierName})
            </h3>
            <p className="text-xs sm:text-sm text-stone-700 mt-1 leading-relaxed max-w-2xl">
              {explanation.rationale}
            </p>
            <div className="flex items-center gap-3 text-xs text-stone-600 font-medium mt-2">
              <span className="inline-flex items-center gap-1 text-emerald-800 font-bold">
                <ShieldCheck className="w-3.5 h-3.5 text-emerald-700" />
                ~₹{monthlyAmount}/month estimate
              </span>
              <span>•</span>
              <span className="text-stone-500">{explanation.percentageNote}</span>
            </div>
          </div>
        </div>
        ...
        <div className="w-full sm:w-auto shrink-0 flex sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-2 pt-3 sm:pt-0 border-t sm:border-t-0 border-stone-200/80">
          {isCurrentlySelected ? (
            ...
            >
              <span>Switch to ₹{recommendedAmount}/day</span>
