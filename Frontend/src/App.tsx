...
import { LandingPage } from './pages/public/NewLandingPage';
...
function HomeRoute() {
  ...
  return (
    <LandingPage
      onGetStarted={() => navigate('/choose-role')}
...
function RoleSelectionRoute() {
  const navigate = useNavigate();
  const { enterDemoRole } = useAuth();

  const handleSelectRole = (role: 'WORKER' | 'CUSTOMER' | 'ADMIN') => {
    const target = role.toLowerCase() as 'worker' | 'customer' | 'admin';
    // Demo navigation only: existing dashboard components are reused as-is.
    enterDemoRole(target);
          ...
          <AppProvider>
            <div className="min-h-screen flex flex-col bg-gray-50 text-gray-900 font-sans antialiased selection:bg-emerald-500 selection:text-white">
              <div className="flex-1 flex flex-col">
                <Routes>
                  {/* NEW Landing_Page is now the root landing page, without the old public header/footer. */}
                  <Route path="/" element={<HomeRoute />} />
                  <Route path="/choose-role" element={<RoleSelectionRoute />} />
                  ...
                  <Route path="/admin" element={<ProtectedRoute roles={['admin']}><AdminLayout /></ProtectedRoute>}>
                    ...
                    <Route path="verification" element={<AdminWorkers />} />
                    <Route path="skills" element={<AdminWorkers />} />
                    <Route path="workforce" element={<AdminWorkers />} />
                    ...
                    <Route path="demand-forecast" element={<AdminDashboard />} />
