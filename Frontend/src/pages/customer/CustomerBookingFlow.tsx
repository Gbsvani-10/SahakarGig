...
export const CustomerBookingFlow: React.FC = () => {
  ...
  const { services, workers, createBooking, addToast, customerLocation, setCustomerLocation } = useApp();
  const { user } = useAuth();
  ...
  const [address, setAddress] = useState(customerLocation.address || 'Flat 402, Lotus Panache, Sector 110');
  ...
  const [step2ViewMode, setStep2ViewMode] = useState<'cards' | 'map'>('cards');

  const [confirmedBookingId, setConfirmedBookingId] = useState<string | null>(null);

  const activeService = services.find((s) => s.id === selectedServiceId) || services[0];
  
  // Available workers for this service category with real computed distances from customer location
  const matchingWorkers = workers
    .filter((w) => !activeService || w.primaryCategory === activeService.category)
    .map((w) => {
      const realDist = (typeof w.latitude === 'number' && typeof w.longitude === 'number')
        ? calculateHaversineDistanceKm(customerLocation.latitude, customerLocation.longitude, w.latitude, w.longitude)
        : (w.distanceKm || 2.5);
      return {
        ...w,
        computedDistanceKm: Math.round(realDist * 10) / 10
      };
    })
    .sort((a, b) => a.computedDistanceKm - b.computedDistanceKm);

  const activeWorker = matchingWorkers.find((w) => w.id === selectedWorkerId) || workers.find((w) => w.id === selectedWorkerId) || matchingWorkers[0] || workers[0];
    ...
    <div className="max-w-4xl mx-auto space-y-6">
        ...
        <Card className="p-6 space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-2 border-b border-gray-100">
            <div>
              ...
              <p className="text-xs text-slate-500">Sorted by proximity to your service address</p>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-2.5 py-1.5 bg-emerald-50 text-emerald-800 border border-emerald-200 rounded-lg text-xs font-semibold hover:bg-emerald-100 transition flex items-center gap-1"
              >
                <MapPin className="w-3.5 h-3.5 text-emerald-600" />
                <span className="max-w-[140px] truncate">{customerLocation.address || 'Change Location'}</span>
              </button>
              ...
              <div className="flex items-center p-0.5 bg-slate-100 rounded-lg border border-slate-200">
                <button
                  ...
                  className={`px-2.5 py-1 text-xs font-bold rounded-md transition ${
                    step2ViewMode === 'cards' ? 'bg-white text-emerald-700 shadow-xs' : 'text-slate-600'
                  }`}
                >
                  Cards
                </button>
                <button
                  type="button"
                  onClick={() => setStep2ViewMode('map')}
                  className={`px-2.5 py-1 text-xs font-bold rounded-md transition ${
                    step2ViewMode === 'map' ? 'bg-white text-emerald-700 shadow-xs' : 'text-slate-600'
                  }`}
                >
                  Map
                </button>
          ...
          {step2ViewMode === 'map' ? (
            <div className="space-y-2">
              <div className="text-xs text-slate-500">Click any artisan marker on the map to select:</div>
              <ServiceMap
                centerLat={customerLocation.latitude}
                centerLng={customerLocation.longitude}
                customerAddress={customerLocation.address}
                radiusKm={10}
                workers={(matchingWorkers.length > 0 ? matchingWorkers : workers).map((w) => ({
                  worker: w,
                  distanceKm: (w as any).computedDistanceKm || w.distanceKm
                }))}
              ...
              {selectedWorkerId && (
                <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-200 flex items-center justify-between text-xs">
                  <span className="font-bold text-emerald-900">
                    Selected: {activeWorker.name} ({activeWorker.primaryCategory})
                  </span>
                  <span className="font-semibold text-emerald-700">
                    📍 {(activeWorker as any).computedDistanceKm || activeWorker.distanceKm} km away
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {(matchingWorkers.length > 0 ? matchingWorkers : workers).map((w: any) => (
                <div
                  key={w.id}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between text-xs pt-1 border-t border-gray-100">
                      ...
                      <span className="text-emerald-700 font-bold bg-emerald-50 px-2 py-0.5 rounded">
                        📍 {w.computedDistanceKm || w.distanceKm} km away
                      </span>
                    </div>
                  </div>
      ...
      <LocationPickerModal
        ...
        onSaveLocation={(loc) => {
          ...
          addToast('success', 'Location Updated', `Search adjusted to ${loc.address}`);
        }}
      />
            ...
            <button
              ...
              onClick={() => {
                if (customerLocation.address) {
                  ...
                  addToast('info', 'Address Filled', 'Customer location populated from GPS.');
                }
              }}
