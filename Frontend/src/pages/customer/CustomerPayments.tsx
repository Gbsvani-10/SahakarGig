...
export const CustomerPayments: React.FC = () => {
  ...
  const pendingBooking = bookings.find((b) => b.paymentStatus !== 'Paid' && b.status === 'Service Completed');
  ...
  const handleProcessPayment = async () => {
    ...
    setProcessing(true);
    try {
      const result = await paymentService.processPayment(pendingBooking.id);
      setPaidTxn(result.transactionId);
      setInvoiceId(result.invoiceId);
      addToast('success', 'Payment Successful', `₹${amount} was captured by Razorpay and verified by SahakarGig.`);
      await refreshData();
    } catch (error: any) {
      addToast('error', 'Payment Failed', error?.message || 'The payment was not completed. Please try again.');
    } finally {
      setProcessing(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-xs sm:text-sm text-gray-500">Real payments through Razorpay with server-side verification</p>
      </div>
      ...
      {paidTxn ? (
        <Card className="p-8 text-center space-y-6 border-emerald-300">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-gray-500">Razorpay Payment ID: {paidTxn}</p>
          ...
          <div className="p-4 rounded-xl bg-emerald-50/70 border border-emerald-200 text-xs text-emerald-950 max-w-md mx-auto space-y-2 text-left">
            ...
            <div className="flex justify-between"><span>Payment Mode:</span><span className="font-bold">Razorpay Checkout</span></div>
          </div>
          <div className="flex items-center justify-center gap-3">
            <Button variant="outline" size="sm" onClick={() => addToast('info', 'Invoice Ready', `Invoice ${invoiceId} is stored in PostgreSQL.`)} leftIcon={<Download className="w-4 h-4" />}>View Receipt</Button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          <div className="md:col-span-7 space-y-4">
            <Card className="p-6 space-y-4">
              <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wider">Choose in Razorpay Checkout</h3>
              ...
              <p className="text-[11px] text-gray-500">Your selected method is a preference for the checkout. Razorpay securely presents the available payment methods and handles the actual payment.</p>
              <Button variant="primary" size="lg" className="w-full mt-4" isLoading={processing} onClick={handleProcessPayment} rightIcon={<ArrowRight className="w-4 h-4" />}>Pay ₹{amount} Securely</Button>
            </Card>
            <div className="flex items-center justify-center gap-2 text-xs text-gray-500"><ShieldCheck className="w-4 h-4 text-emerald-700" /><span>Payment is verified server-side before it is marked paid</span></div>
          </div>
          ...
          <div className="md:col-span-5 space-y-4">
            <Card className="p-6 space-y-4 bg-gray-50">
              ...
              <div className="p-3 bg-emerald-100/60 rounded-xl border border-emerald-200 text-xs text-emerald-950 space-y-1"><p className="font-bold flex items-center gap-1"><HeartHandshake className="w-3.5 h-3.5 text-emerald-700" />Empowering Cooperative Workers</p><p className="text-[11px] text-emerald-900">The final amount is taken from the completed booking on the server; the browser cannot change the payable amount.</p></div>
            </Card>
          </div>
        </div>
      )}
