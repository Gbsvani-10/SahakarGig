...
export const EarningsCard: React.FC<EarningsCardProps> = ({ worker }) => {
  const hasEarnings =
    worker.dailyEarnings !== null &&
    worker.dailyEarnings !== undefined;

  const hasMonthlyEarnings =
    worker.estimatedMonthlyEarnings !== null &&
    worker.estimatedMonthlyEarnings !== undefined;

  const hasWorkingDays =
    worker.estimatedWorkingDays !== null &&
    worker.estimatedWorkingDays !== undefined;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-base font-bold text-stone-900 mt-0.5">
              {worker.name || 'Data not available'} •{' '}
              <span className="font-normal text-stone-600">
                {worker.occupation || 'Data not available'}
              </span>
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4">
        ...
        >
          <div>
            ...
            <div className="text-xs text-stone-500 mt-0.5">
              {hasMonthlyEarnings && hasWorkingDays
                ? `Calculated from ${worker.estimatedWorkingDays} active working days`
                : hasMonthlyEarnings
                  ? 'Working-day information not available'
                  : 'Subject to active shifts logged'}
            </div>
          </div>

          <div className="text-right">
            {hasEarnings && hasWorkingDays ? (
              <span className="inline-block px-2.5 py-1 rounded-full text-xs font-semibold bg-stone-100 text-stone-700">
                ₹{worker.dailyEarnings!.toLocaleString('en-IN')} ×{' '}
                {worker.estimatedWorkingDays}d
              </span>
            ) : (
              <span className="inline-block px-2.5 py-1 rounded-full text-xs font-semibold bg-stone-100 text-stone-500">
      ...
      <div className="mt-3.5 flex items-start gap-2 p-3 rounded-xl bg-stone-50 border border-stone-200/80 text-xs text-stone-600">
        ...
        <p>
          ...
          <strong className="text-stone-800">
            {worker.workerId || 'Data not available'}
          </strong>{' '}
          • Employer/Platform:{' '}
          <strong className="text-stone-800">
            {worker.employer || 'Data not available'}
          </strong>{' '}
          • Location:{' '}
          <strong className="text-stone-800">
            {worker.location || 'Data not available'}
          </strong>
          . Earnings records are synced securely from the SahakarGig partner
          platform.
        </p>
      </div>
    </div>
