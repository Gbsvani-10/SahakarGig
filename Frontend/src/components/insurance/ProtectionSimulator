...
import { ContributionAmount, ClaimType } from '../../types/insurance';
...
interface ProtectionSimulatorProps {
  ...
  onStartClaimFromScenario?: (scenarioType: ClaimType) => void;
}
...
export const ProtectionSimulator: React.FC<ProtectionSimulatorProps> = ({
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-2xl bg-white rounded-2xl shadow-xl border border-stone-200 overflow-hidden my-6">
        {/* Header */}
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-stone-400">
                Selected Plan: ₹{selectedContribution}/day ({protectionTier})
              </span>
            </div>
        ...
        <div className="p-5 sm:p-6 space-y-5 max-h-[75vh] overflow-y-auto">
          ...
          <div>
            <label className="block text-sm font-bold text-stone-900 mb-2">
              What happened? <span className="font-normal text-stone-500">(Choose a scenario)</span>
            </label>
                ...
                return (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="text-[10px] text-stone-500 line-clamp-1">{s.tagline}</div>
                    </div>
                  </button>
                );
              })}
          ...
          <div className="rounded-xl border border-stone-200 bg-stone-50/70 p-4 sm:p-5 space-y-4">
            ...
            <div className="p-3 rounded-lg bg-amber-50/80 border border-amber-200 flex items-start gap-2 text-xs text-amber-900">
              ...
              <p>
                <span className="font-semibold">Illustrative disclaimer:</span> Micro-insurance provides
                transparent reimbursement support subject to verified medical receipts and policy rules.
                We never show false cash-advance guarantees.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 sm:p-5 bg-stone-50 border-t border-stone-200 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          {onStartClaimFromScenario && (
            <button
              ...
              onClick={() => {
                onClose();
                onStartClaimFromScenario(currentScenario.id as ClaimType);
              }}
