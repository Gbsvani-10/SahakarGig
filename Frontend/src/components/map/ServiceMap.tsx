...
export const ServiceMap: React.FC<ServiceMapProps> = ({
  ...
  useEffect(() => {
    ...
    const centerMarker = L.marker([centerLat, centerLng], { icon: centerIcon }).addTo(markerLayer);
    if (customerAddress) {
      centerMarker.bindPopup(`<b>Your Location</b><br/>${escapeHtml(customerAddress)}`);
    }
    ...
    workers.forEach((item) => {
      ...
      const workerIcon = L.divIcon({
        ...
        html: `<div style="background:${isSelected ? '#dc2626' : '#2563eb'};color:white;width:${markerSize}px;height:${markerSize}px;border-radius:50%;display:flex;align-items:center;justify-content:center;border:2px solid white;box-shadow:0 2px 5px rgba(0,0,0,0.25);font-size:12px;cursor:pointer;">🛠️</div>`,
        iconSize: [markerSize, markerSize],
        iconAnchor: [markerSize / 2, markerSize / 2],
      });

      const marker = L.marker([latitude, longitude], { icon: workerIcon }).addTo(markerLayer);
      const workerName = worker.name || 'Worker';
      const trade = worker.primaryTrade || 'Service Professional';
      const rating = Number(worker.rating);
      const safeRating = Number.isFinite(rating) ? rating.toFixed(1) : '—';
      const distance = item.distanceKm !== undefined && Number.isFinite(Number(item.distanceKm))
        ? `<br/>Distance: ${Number(item.distanceKm).toFixed(1)} km`
        : '';

      marker.bindPopup(`<div><b>${escapeHtml(workerName)}</b><br/>Trade: ${escapeHtml(trade)}<br/>Rating: ⭐ ${safeRating}${distance}</div>`);
