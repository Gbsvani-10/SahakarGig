...
export const WorkerAuth: React.FC<WorkerAuthProps> = ({ onSuccess, onBack }) => {
  ...
  const [expectedDailyWage, setExpectedDailyWage] = useState(0);
  ...
  const [certifications, setCertifications] = useState<
    { skill: string; name: string; organization: string; year: string; certificateFileName?: string; certificateFileData?: string }[]
  >([]);
  ...
  const [certSkillName, setCertSkillName] = useState('');
  ...
  const handleNextStep = () => {
    if (step === 1) {
      ...
      if (password !== confirmPassword) return error('Validation Error', 'Password and Confirm Password do not match');
      ...
      if (!jobType) return error('Validation Error', 'Please select your primary job');
  ...
  const handleAddCertification = async () => {
    ...
    setCertifications([
      ...certifications,
      {
        ...
        certificateFileData: certFile ? await readFile(certFile) : undefined
      }
    ]);
    setCertCourseName('');
    setCertOrg('');
  ...
  const handleCompleteOnboarding = async (e: React.FormEvent) => {
    ...
    if (!identityProofFile) { error('Identity Proof Required', 'Please upload your selected identity proof.'); return; }
    setLoading(true);
    try {
      const identityProofData = await readFile(identityProofFile);
      const res = await api.registerWorker({
        ...
        identityProofFileName: identityProofFile.name,
        identityProofData,
        emergencyContact: {
          ...
          <div className="space-y-6 text-left">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      id="worker-reg-confirm-password"
                      type="password"
                      required
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-900 focus:bg-white focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600"
                    >
                      <option value="">Select your primary job</option>
            ...
            {step === 3 && (
              <form onSubmit={handleCompleteOnboarding} className="space-y-4">
                ...
                <div>
                  <label className="block text-[11px] font-semibold text-slate-700 mb-1">Upload Identity Proof * (PDF, JPG or PNG)</label>
                  <input type="file" accept="application/pdf,image/jpeg,image/png" required onChange={(e) => setIdentityProofFile(e.target.files?.[0] || null)} className="w-full text-xs text-slate-600" />
                  {identityProofFile && <p className="text-[11px] text-emerald-700 mt-1">Selected: {identityProofFile.name}</p>}
                </div>
                ...
                <div className="p-4 rounded-xl border border-slate-200 bg-slate-50 space-y-3">
                  ...
                  <div>
                    <label className="block text-[10px] font-medium text-slate-600 mb-0.5">Certificate File (optional)</label>
                    <input type="file" accept="application/pdf,image/jpeg,image/png" onChange={(e) => setCertFile(e.target.files?.[0] || null)} className="w-full text-xs text-slate-600" />
                  </div>
