...
export const NearbyWorkerCard: React.FC<NearbyWorkerCardProps> = ({
  ...
  const initials = workerName.trim().split(/\s+/).filter(Boolean).map((name) => name[0]).join('').slice(0, 2).toUpperCase();
  ...
  return (
    <div
      ...
      className={`p-4 rounded-xl border transition-all cursor-pointer bg-white ${
        isSelected ? 'border-emerald-500 ring-2 ring-emerald-500/20 shadow-md' : 'border-slate-200 hover:border-slate-300 hover:shadow-sm'
      }`}
    >
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-1.5 font-bold text-slate-900 text-sm">
              ...
              {worker.verificationStatus === 'Verified' && (
                <ShieldCheck className="w-4 h-4 text-emerald-600 flex-shrink-0" aria-label="Verified worker" />
              )}
            </div>
            <div className="text-xs text-slate-500 capitalize truncate">{worker.primaryTrade || 'Service Professional'}</div>
          </div>
        </div>
        <div className="flex flex-col items-end gap-1 flex-shrink-0">
          {isClosest && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800">Closest</span>}
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-slate-600 pt-2 border-t border-slate-100">
        ...
        <div className="flex items-center gap-1 justify-end">
          ...
          <span>{typeof distanceKm === 'number' && Number.isFinite(distanceKm) ? `${distanceKm.toFixed(1)} km away` : 'Nearby'}</span>
        </div>
      </div>
