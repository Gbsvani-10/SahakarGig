...
export const AdminAuth: React.FC<AdminAuthProps> = ({ onSuccess, onBack }) => {
  ...
  const [identifier, setIdentifier] = useState('admin@sahakargig.in');
    ...
    <div className="min-h-[85vh] py-12 px-4 sm:px-6 lg:px-8 bg-slate-50 flex items-center justify-center">
      <div className="max-w-md w-full bg-white rounded-2xl border border-slate-200 shadow-xl shadow-slate-900/5 p-8 text-left">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Quick test helper for demonstration / SIH judges */}
          <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl text-[11px] text-slate-600 space-y-1">
            <div className="flex items-center gap-1.5 font-semibold text-slate-700">
              <KeyRound className="w-3.5 h-3.5 text-indigo-600" /> Demo Admin Credentials:
            </div>
            <p>Email: <code className="bg-white px-1.5 py-0.5 rounded border text-slate-800">admin@sahakargig.in</code></p>
            <p>Password: <code className="bg-white px-1.5 py-0.5 rounded border text-slate-800">Admin@2026</code></p>
          </div>
